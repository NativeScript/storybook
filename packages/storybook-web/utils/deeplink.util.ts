...
export function listenToStoryChange(targetPlatform: 'android' | 'ios'): void {
  addons.ready().then((channel) => {
    setTimeout(() => {
      ...
      combineLatest([component$.pipe(distinctUntilChanged()), queryParams$.pipe(distinctUntilChanged())])
...
function updateDeepLink(component: string, storyParams: Record<string, any>, targetPlatform: 'android' | 'ios'): void {
  ...
  if (storyParams?.args) {
    ...
    const newAppUrl = getFullDeepLinkUrl(deepLinkBaseUrl, storyParamsWithExtras);
    ...
    const newAppUrl = getFullDeepLinkUrl(deepLinkBaseUrl, {
      component: component,
    });
    console.groupCollapsed('Generating deeplink');
    console.log('newAppUrl', newAppUrl);
  ...
  return Object.keys(data)
    .map((key) => {
      if (typeof data[key] === 'object') {
        const subData = Object.keys(data[key])
          .map((subKey) => {
