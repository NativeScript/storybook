import { ExecutorContext, logger, readJson, readJsonFile, readProjectConfiguration, TargetConfiguration, Tree } from '@nrwl/devkit';
...
export function isFramework(type: Framework['type'], schema: Pick<Framework, 'uiFramework'>) {
  ...
  if (type === 'web-components' && schema.uiFramework === '@storybook/web-components') {
    return true;
  }
  ...
  if (type === 'react-native' && schema.uiFramework === '@storybook/react-native') {
    return true;
  }
...
export function readCurrentWorkspaceStorybookVersionFromGenerator(tree: Tree): string {
...
function determineStorybookWorkspaceVersion(packageJsonContents) {
  ...
  if (packageJsonContents && packageJsonContents['devDependencies']) {
    ...
    if (packageJsonContents['devDependencies']['@storybook/react-native']) {
      workspaceStorybookVersion = packageJsonContents['devDependencies']['@storybook/react-native'];
    }
  }
  ...
  if (packageJsonContents && packageJsonContents['dependencies']) {
    ...
    if (packageJsonContents['dependencies']['@storybook/react-native']) {
      workspaceStorybookVersion = packageJsonContents['dependencies']['@storybook/react-native'];
    }
  }
...
export function findOrCreateConfig(config: StorybookConfig, context: ExecutorContext): string {
  if (config?.configFolder && statSync(config.configFolder).isDirectory()) {
    return config.configFolder;
  } else if (statSync(config.configPath).isFile() && statSync(config.pluginPath).isFile() && statSync(config.srcRoot).isFile()) {
...
function createStorybookConfig(configPath: string, pluginPath: string, srcRoot: string): string {
  const tmpDir = tmpdir();
  const tmpFolder = `${tmpDir}${sep}`;
  mkdtempSync(tmpFolder);
  copyFileSync(configPath, `${tmpFolder}/${basename(configPath)}`, constants.COPYFILE_EXCL);
  copyFileSync(pluginPath, `${tmpFolder}/${basename(pluginPath)}`, constants.COPYFILE_EXCL);
  copyFileSync(srcRoot, `${tmpFolder}/${basename(srcRoot)}`, constants.COPYFILE_EXCL);
  return tmpFolder;
}
...
export function findStorybookAndBuildTargetsAndCompiler(targets: { [targetName: string]: TargetConfiguration }): {
...
export function isTheFileAStory(tree: Tree, path: string): boolean {
  ...
  if (ext === '.tsx' || ext === '.ts') {
    ...
    importArray.forEach((importNode: ts.ImportClause) => {
      ...
      const importSpecifiers = findNodes(importNode, [ts.SyntaxKind.ImportSpecifier]);
      importSpecifiers.forEach((importSpecifier: ts.ImportSpecifier) => {
        if (importSpecifier.getText() === 'Story' || importSpecifier.getText() === 'storiesOf' || importSpecifier.getText() === 'ComponentStory') {
    ...
    fileIsStory = (path.endsWith('.js') && path.endsWith('.stories.js')) || (path.endsWith('.jsx') && path.endsWith('.stories.jsx'));
  }
