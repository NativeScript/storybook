import { generateFiles, joinPathFragments, logger, offsetFromRoot, readJson, readProjectConfiguration, toJS, Tree, updateJson, updateProjectConfiguration, writeJson } from '@nrwl/devkit';
import { Linter } from '@nrwl/linter';
import { join } from 'path';
import { dedupe, findStorybookAndBuildTargetsAndCompiler, isFramework, TsConfig } from '../../utils/utilities';
import { InitGeneratorSchema } from './schema';
import { getRootTsConfigPathInTree } from '@nrwl/workspace/src/utilities/typescript';

export function addStorybookTask(tree: Tree, projectName: string, uiFramework: string) {
...
export function configureTsProjectConfig(tree: Tree, schema: InitGeneratorSchema) {
...
export function configureTsSolutionConfig(tree: Tree, schema: InitGeneratorSchema) {
  const { name: projectName } = schema;
  ...
  if (!tsConfigContent.references?.map((reference) => reference.path)?.includes('./.storybook/tsconfig.json')) {
 ...
 */
export function updateLintConfig(tree: Tree, schema: InitGeneratorSchema) {
  const { name: projectName } = schema;
  ...
  tslintTargets.forEach((target) => {
    target.options.tsConfig = dedupe([...target.options.tsConfig, joinPathFragments(root, './.storybook/tsconfig.json')]);
  });
  ...
  if (tree.exists(join(root, '.eslintrc.json'))) {
    updateJson(tree, join(root, '.eslintrc.json'), (json) => {
      ...
      if (Array.isArray(json.parserOptions?.project)) {
        json.parserOptions.project = dedupe([...json.parserOptions.project, join(root, '.storybook/tsconfig.json')]);
      }
      ...
      for (const o of overrides) {
        ...
        if (Array.isArray(o.parserOptions?.project)) {
          o.parserOptions.project = dedupe([...o.parserOptions.project, join(root, '.storybook/tsconfig.json')]);
        }
      }
...
export function normalizeSchema(schema: InitGeneratorSchema): InitGeneratorSchema {
  const defaults = {
...
export function createRootStorybookDir(tree: Tree, js: boolean, tsConfiguration: boolean) {
  if (tree.exists('.storybook')) {
    logger.warn(`.storybook folder already exists at root! Skipping generating files in it.`);
    return;
  }
  logger.debug(`adding .storybook folder to the root directory`);

  const templatePath = join(__dirname, tsConfiguration ? './root-files-ts' : './root-files');
...
export function createProjectStorybookDir(tree: Tree, projectName: string, uiFramework: InitGeneratorSchema['uiFramework'], js: boolean, tsConfiguration: boolean, isNextJs?: boolean, usesSwc?: boolean) {
  ...
  const projectDirectory = projectType === 'application' ? (isNextJs ? 'components' : 'src/app') : 'src/lib';

  const storybookRoot = join(root, '.storybook');

  if (tree.exists(storybookRoot)) {
    logger.warn(`.storybook folder already exists for ${projectName}! Skipping generating files in it.`);
    return;
  }

  logger.debug(`adding .storybook folder to your ${projectType}`);
  const templatePath = join(__dirname, tsConfiguration ? './project-files-ts' : './project-files');
  ...
  generateFiles(tree, templatePath, root, {
    ...
    rootTsConfigPath: getRootTsConfigPathInTree(tree),
    projectDirectory,
    useWebpack5: uiFramework === '@storybook/angular' || uiFramework === '@storybook/vue',
...
export function getTsConfigPath(tree: Tree, projectName: string, path?: string): string {
  const { root, projectType } = readProjectConfiguration(tree, projectName);
  return join(root, path && path.length > 0 ? path : projectType === 'application' ? 'tsconfig.app.json' : 'tsconfig.lib.json');
}
    ...
    tasksRunnerOptions: {
      ...
      default: {
        ...
        options: {
          ...
          cacheableOperations: Array.from(new Set([...(json.tasksRunnerOptions?.default?.options?.cacheableOperations ?? []), 'build-storybook'])),
        },
      },
    },
  }));
