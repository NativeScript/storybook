import { StorybookUrl } from '../models';
...
export class UrlUtil {
  ...
  handleAndroidIntent(intent: android.content.Intent): StorybookUrl {
    ...
    try {
      ...
      if (appUrl != null && (new String(intent.getAction()).valueOf() === new String(android.content.Intent.ACTION_MAIN).valueOf() || new String(intent.getAction()).valueOf() === new String(android.content.Intent.ACTION_VIEW).valueOf())) {
        try {
  ...
  private static _instance: UrlUtil = new UrlUtil();
  static getInstance(): UrlUtil {
